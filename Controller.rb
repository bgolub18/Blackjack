...
get '/' do
	...
	@hand = "You\'re hand is the #{session[:game].value(session[:game].your_hand[0])} and the #{session[:game].value(session[:game].your_hand[1])} for a score of #{session[:game].score}"
...
post '/' do
	...
	case params[:choice]
	when "hit"
		@hand = "You\'re score is now #{session[:game].score}."
		if session[:game].score > 21
